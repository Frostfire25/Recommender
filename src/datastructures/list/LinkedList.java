 ...
 */
 public class LinkedList<T> implements ListInterface<T>
 {
   ...
   */
   ...
   public String toString()
   {
     ...
     try
     {
       for (int i = 1; i <= itemCount; i++)
        ret += "[" + i + "]: " + getNodeAt(i).getItem() + "\n";
     }
     catch (IndexOutOfBoundsException iex)
     {
       System.out.println("LinkedList.toString(): index out of bounds.");
       iex.printStackTrace();
       System.exit(1);
     }
     return ret;
   ...
   /**
    * Truncates a list to be of length {@code max(0, newLength)}.
    * If {@code newLength} is greater than the current length, we don't
    * modify the list.
    *
